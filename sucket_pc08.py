...
def stream_decoder():
    ...
    while decoder_going:
        ...
        while len(image.stream_queue) > 0:
            time_begin = time.time()                        # perf measure *********************
            work_part = image.stream_queue.popleft()

            while not "STOP" in work_part: # part has no comlete pickle
                work_part += image.stream_queue.popleft()
                nostop += 1

            while "STOP" in work_part:
                data_parts = work_part.partition("STOP")
                try:
                    data = cPickle.loads(data_parts[0])
                    image.pickles_decoded_count += 1
                except:
                    image.pickles_failed += 1
                image.data_queue.append(data)
                image.latest_data = data
                work_part = data_parts[2]

            image.pickle_decode_time = time.time() - time_begin # perf measure ******************

        if image.finished and len(image.stream_queue) == 0:
            decoder_going = False

    print "decoder end"
    print "nostops", nostop
...
def worker():
    ...
    while going:
        ...
        datasize_text = "Data " + str(len(image.data_queue))
    ...
    print "pickles failed:", image.pickles_failed
